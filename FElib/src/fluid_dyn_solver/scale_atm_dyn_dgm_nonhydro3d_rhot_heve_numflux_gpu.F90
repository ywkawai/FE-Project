...
module scale_atm_dyn_dgm_nonhydro3d_rhot_heve_numflux_gpu
  ...
  public :: atm_dyn_dgm_nonhydro3d_rhot_heve_numflux_get_generalhvc_gpu
...
contains
  ...
  subroutine atm_dyn_dgm_nonhydro3d_rhot_heve_numflux_get_generalhvc_gpu( &
    del_flux,                                                        & ! (out)
    DDENS_, MOMX_, MOMY_, MOMZ_, DRHOT_, DPRES,                      & ! (in)
    DENS_hyd, PRES_hyd, THERM_hyd, Rtot, CVtot, CPtot,               & ! (in)
    ...
    do ke=NeS, NeE
      ...
      do fp=1, NfpTot
        ...
        del_flux(fp,MOMX_VID,ke) = fscale_ * ( &
                        GsqrtMOMX_P * Vel_P - GsqrtMOMX_M * Vel_M &
                      + ( G1n_P + Gxz_P * nz_ ) * GsqrtDPres_P   &
                      - ( G1n_M + Gxz_M * nz_ ) * GsqrtDPres_M   &
                      - alpha * ( GsqrtMOMX_P - GsqrtMOMX_M )  )

        del_flux(fp,MOMY_VID,ke) = fscale_ * ( &
                        GsqrtMOMY_P * Vel_P - GsqrtMOMY_M * Vel_M &
                      + ( G2n_P + Gyz_P * nz_ ) * GsqrtDPres_P   &
                      - ( G2n_M + Gyz_M * nz_ ) * GsqrtDPres_M   &
                      - alpha * ( GsqrtMOMY_P - GsqrtMOMY_M ) )
      end do
    end do
    !$acc end parallel
    ...
    return
  end subroutine atm_dyn_dgm_nonhydro3d_rhot_heve_numflux_get_generalhvc_gpu  

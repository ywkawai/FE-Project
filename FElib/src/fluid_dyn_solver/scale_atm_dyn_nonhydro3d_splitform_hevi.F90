...
module scale_atm_dyn_nonhydro3d_splitform_hevi
  ...
  !++ Public procedures
  !
  public :: atm_dyn_nonhydro3d_hevi_splitform_Init
  public :: atm_dyn_nonhydro3d_hevi_splitform_Final
  public :: atm_dyn_nonhydro3d_hevi_splitform_cal_tend
  public :: atm_dyn_nonhydro3d_hevi_splitform_cal_vi
...
contains
  subroutine atm_dyn_nonhydro3d_hevi_splitform_Init( mesh )
    ...
    return
  end subroutine atm_dyn_nonhydro3d_hevi_splitform_Init


  subroutine atm_dyn_nonhydro3d_hevi_splitform_Final()
    implicit none
    !--------------------------------------------
    ...
    return
  end subroutine atm_dyn_nonhydro3d_hevi_splitform_Final  
  ...
  subroutine cal_del_flux_dyn( del_flux, &
    ...
    real(RP) :: dens, u, v, w, pot, velh, vel
    real(RP) :: gamm, rgamm
    ...
    !$omp parallel do private( iM, iP, alpha, swV, &
    !$omp VelP, VelM, MOMZ_P, presM, presP, dpresM, dpresP,   &
    !$omp densM, densP, rhotM, rhotP, rhot_hyd_M, rhot_hyd_P, & 
    !$omp dens, u, v, w, pot, velh, vel )
    do i=1, elem%NfpTot*lmesh%Ne
      ...
      u = 0.5_RP * (MOMX_(iM) + MOMX_(iP)) / dens
      v = 0.5_RP * (MOMY_(iM) + MOMY_(iP)) / dens
      w = 0.5_RP * (MOMZ_(iM) + MOMZ_P   ) / dens
      velh = u*nx(i) + v*ny(i)
      vel = velh + w*nz(i)

      pot = 0.5_RP * (rhotM + rhotP) / dens

      alpha = swV*max( sqrt(gamm * presM / densM) + abs(VelM), &
                       sqrt(gamm * presP / densP) + abs(VelP)  )
            
      del_flux(i,DDENS_VID) = &
                    (dens*u - MOMX_(iM)) * nx(i)             &
                  + (dens*v - MOMY_(iM)) * ny(i)             &
                  - 0.5_RP * alpha * (DDENS_(iP) - DDENS_(iM)) 
    
      del_flux(i,MOMX_VID) = &
                    ( dens*u*vel - MOMX_(iM)*VelM )         &
                  + ( dpresP - dpresM )*nx(i)               &
                  - 0.5_RP * alpha * (MOMX_(iP) - MOMX_(iM))     

      del_flux(i,MOMY_VID) = &
                    ( dens*v*vel - MOMY_(iM)*VelM )        &
                  + ( dpresP - dpresM )*ny(i)              &
                  - 0.5_RP * alpha * (MOMY_(iP) - MOMY_(iM))     

      del_flux(i,MOMZ_VID) = &
                    ( dens*w*vel - MOMZ_(iM)*VelM )        &
                  - 0.5_RP * alpha * (MOMZ_(iP) - MOMZ_(iM))      
      
      del_flux(i,DRHOT_VID) = &
                  swV*( dens*pot*vel - rhotM*VelM)           &
                - 0.5_RP * alpha * (DRHOT_(iP) - DRHOT_(iM))
  ...
  subroutine atm_dyn_nonhydro3d_hevi_splitform_cal_vi( &
    ...
    return
  end subroutine atm_dyn_nonhydro3d_hevi_splitform_cal_vi
...
end module scale_atm_dyn_nonhydro3d_splitform_hevi

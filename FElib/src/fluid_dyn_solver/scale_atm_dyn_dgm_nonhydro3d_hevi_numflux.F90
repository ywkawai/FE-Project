...
module scale_atm_dyn_dgm_nonhydro3d_hevi_numflux
  ...
  public :: atm_dyn_dgm_nonhydro3d_hevi_numflux_get_generalhvc_entot
...
!OCL SERIAL
  subroutine atm_dyn_dgm_nonhydro3d_hevi_numflux_get_generalhvc_entot( &
    del_flux, del_flux_hyd,                                          & ! (out)
    DDENS_, MOMX_, MOMY_, MOMZ_, DRHOT_, DENS_hyd, PRES_hyd,         & ! (in)
    Rtot, CVtot, CPtot,                                              & ! (in)
    ...
    real(RP) :: GsqrtDensM(elem%NfpTot), GsqrtDensP(elem%NfpTot)
    real(RP) :: GsqrtRhotM(elem%NfpTot), GsqrtRhotP(elem%NfpTot)
    real(RP) :: GsqrtEnTotM(elem%NfpTot), GsqrtEnTotP(elem%NfpTot)
    ...
    real(RP) :: u1M(elem%NfpTot), u2M(elem%NfpTot)
    real(RP) :: u1P(elem%NfpTot), u2P(elem%NfpTot)
    ...
    !$omp GsqrtMOMX_M, GsqrtMOMX_P, GsqrtMOMY_M, GsqrtMOMY_P, GsqrtMOMZ_M, GsqrtMOMZ_P, &
    !$omp GsqrtDDENS_M, GsqrtDDENS_P, GsqrtDRHOT_M, GsqrtDRHOT_P,                       &
    !$omp GsqrtEnTotM, GsqrtEnTotP, GsqrtEnthalpyM, GsqrtEnthalpyP, u1M, u2M, u1P, u2P, &
    !$omp Phyd_M, Phyd_P,                                                               &
    !$omp Gsqrt_P, Gsqrt_M, GsqrtV_P, GsqrtV_M, G13_P, G13_M, G23_P, G23_M,             &
    !$omp Gxz_P, Gxz_M, Gyz_P, Gyz_M, G1n_M, G2n_M, Gnn_P, Gnn_M, swV                   )
    do ke=lmesh%NeS, lmesh%NeE
      ...
      u1M(:) = ( G_11(iM2Dto3D(:),ke2D) * GsqrtMOMX_M(:) + G_12(iM2Dto3D(:),ke2D) * GsqrtMOMY_M(:) )
      u2M(:) = ( G_12(iM2Dto3D(:),ke2D) * GsqrtMOMX_M(:) + G_22(iM2Dto3D(:),ke2D) * GsqrtMOMY_M(:) )
      u1P(:) = ( G_11(iM2Dto3D(:),ke2D) * GsqrtMOMX_P(:) + G_12(iM2Dto3D(:),ke2D) * GsqrtMOMY_P(:) )
      u2P(:) = ( G_12(iM2Dto3D(:),ke2D) * GsqrtMOMX_P(:) + G_22(iM2Dto3D(:),ke2D) * GsqrtMOMY_P(:) )

      GsqrtEnTotM(:) = GsqrtDensM(:) * Grav * zlev(iM)                                &
                      + Gsqrt_M(:) * CVtot(iM) * ( Phyd_M(:) + dpresM(:) ) / Rtot(iM) &
                      + 0.5_RP * ( GsqrtMOMX_M(:) * u1M(:) + GsqrtMOMY_M(:) * u2M(:) + GsqrtMOMZ_M(:)**2 ) / GsqrtDENSM(:)
      GsqrtEnTotP(:) = GsqrtDensP(:) * Grav * zlev(iM)                                &
                      + Gsqrt_P(:) * CVtot(iP) * ( Phyd_P(:) + dpresP(:) ) / Rtot(iP) &
                      + 0.5_RP * ( GsqrtMOMX_P(:) * u1P(:) + GsqrtMOMY_P(:) * u2P(:) + GsqrtMOMZ_P(:)**2 ) / GsqrtDENSP(:)
      
      GsqrtEnthalpyM(:) = GsqrtEnTotM(:) + Gsqrt_M(:) * ( Phyd_M(:) + dpresM(:) )
      GsqrtEnthalpyP(:) = GsqrtEnTotP(:) + Gsqrt_P(:) * ( Phyd_P(:) + dpresP(:) )
...
!                    ( GsqrtEnthalpyP(:) * VelP(:) - GsqrtEnthalpyM(:) * VelM(:) ) &
    ...
    return
  end subroutine atm_dyn_dgm_nonhydro3d_hevi_numflux_get_generalhvc_entot

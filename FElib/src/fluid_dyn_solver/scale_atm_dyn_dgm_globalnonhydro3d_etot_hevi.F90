...
!OCL SERIAL
  subroutine atm_dyn_dgm_globalnonhydro3d_etot_hevi_cal_tend( &
    ...
    use scale_atm_dyn_dgm_nonhydro3d_hevi_numflux, only: &
      atm_dyn_dgm_nonhydro3d_hevi_numflux_get_generalhvc_entot
    ...
    call atm_dyn_dgm_nonhydro3d_hevi_numflux_get_generalhvc_entot( &
      ...
      lmesh%GsqrtH, lmesh%GI3(:,:,1), lmesh%GI3(:,:,2), lmesh%zlev(:,:),       & ! (in)                       & ! (in)    
...
!      call sparsemat_matmul(Dz, lmesh%Gsqrt(:,ke) * ( wt_(:) * MOMZ_(:,ke) + RGsqrtV(:) * DPRES_(:) ), Fz)
      ...
      call sparsemat_matmul(Dz, lmesh%Gsqrt(:,ke) * ( wt_(:) - 0.0_RP*w_(:) * RGsqrtV(:) ) * enthalpy_(:), Fz)
      Fz(:) = 0.0_RP
...
!OCL SERIAL  
  subroutine atm_dyn_dgm_globalnonhydro3d_etot_hevi_cal_vi( &
    ...
    do ke_z=1, lmesh%NeZ
      ...
      GsqrtV_z(:,ke_z,ke_xy) = 1.0_RP !lmesh%Gsqrt(:,ke) / lmesh%GsqrtH(elem%IndexH2Dto3D,ke2D)
    ...
    if ( abs(impl_fac) > 0.0_RP ) then
      ...
      do itr_nlin = 1, 1
        ...
        do ke_xy=1, lmesh%NeX * lmesh%NeY
          ...
          !$omp parallel private(ij, v, ke_z, info, info_uv, ColMask, DENS_)
          !$omp do

...
contains
  ...
  subroutine MeshUtil2D_genRectDomain( pos_v, EToV, &
      ...
      do i=1, NvX
         n = i + (j-1)*NvX
         pos_v(n,1) = (xmax - xmin)*(i - 1)/dble(NvX - 1) + xmin
         pos_v(n,2) = (ymax - ymin)*(j - 1)/dble(NvY - 1) + ymin
      end do
      end do
...
subroutine MeshUtil2D_genPatchBoundaryMap(  VMapB, MapB, VMapP, &
  ...
  real(RP) :: x, y
   ...
   do f=1, Nfaces
     ...
     call eval_domain_boundary(1, y, ymin, x, k, f)
     call eval_domain_boundary(2, x, xmax, y, k, f)
     call eval_domain_boundary(3, y, ymax, x, k, f)
     call eval_domain_boundary(4, x, xmin, y, k, f)
   end do
   end do
   ...
   contains
     subroutine eval_domain_boundary(domb_id, r, rbc, ord_info, k, f)
...
!    end do
  ...
  subroutine MeshUtil2D_buildGlobalMap( &
    ...
    Ne = sqrt(dble(NtilePerPanel))
    Nv = Ne + 1
    allocate( nodesID_2d(Nv,Nv) )
    ...
    if (isPeriodicX) then
      do tileID=1, Ntile
        if (pi_table(tileID) == 1 .and. tileFaceID_map(4,tileID) == 4) then
          tileID_map(4,tileID) = Ne + (pj_table(tileID) - 1)*Ne
          tileFaceID_map(4,tileID) = 2
        end if
        if (pi_table(tileID) == Ne .and. tileFaceID_map(2,tileID) == 2) then
          tileID_map(2,tileID) = 1 + (pj_table(tileID) - 1)*Ne
    ...
    if (isPeriodicY) then
      do tileID=1, Ntile
        if (pj_table(tileID) == 1 .and. tileFaceID_map(1,tileID) == 1) then
          tileID_map(1,tileID) = pi_table(tileID) + (Ne - 1)*Ne
          tileFaceID_map(1,tileID) = 3
        end if
        if (pj_table(tileID) == Ne .and. tileFaceID_map(3,tileID) == 3) then

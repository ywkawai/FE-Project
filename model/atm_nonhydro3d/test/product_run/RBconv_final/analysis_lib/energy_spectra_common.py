...
def create_zslice_nc(dir, dir_ini, tmp_dir, penum, zlevel_list, time_list, Nproc=4):
  r = Parallel(n_jobs=Nproc, backend="multiprocessing")( [delayed(create_tmp_zslice_nc_sub)(dir, dir_ini, tmp_dir, pe, zlevel_list, time_list) for pe in range(0,penum) ] )
...
def energy_spectra_analysis(exp_top_dir, exp_name, nx, penum, L, 
  ...
  r = Parallel(n_jobs=nproc, backend="multiprocessing")( [delayed(energy_spectra_analysis_core_sub)( tmp_dir, nx, L, di, TIME_list[di], ZLEVEL_list, OUTNC_suffix) for di in dir_ind ] )
...
def xaxis_txt_minor(inv_lam, pos=None):
  ...
  # if l=="20" or l=="30" or l=="40" or l=="50" or l=="200" or l=="500" or l=="2000" or l=="5000":
    return "  "+l+'$^{-1}$'
  else:
    return ""
...
def read_spectra_data(exp_name, tmp_dir, dir_ind, ke_spectra_3dvel_list, 
  ...
  ke_spectra_3dvel_tmp = fac * np.mean(ke_spectra_hvel[start_ind:,:,1:ncut] + ke_spectra_wvel[1:,:,1:ncut], axis=0)
  for i in range(1,len(dir_ind)):
    di = dir_ind[i]
    ke_spectra_hvel = xr.open_mfdataset(f'{tmp_dir}_{di}/KE_spectra_momh{OUTNC_suffix}.nc', decode_times=False, combine='by_coords').KE_spectra_momh
    ke_spectra_wvel = xr.open_mfdataset(f'{tmp_dir}_{di}/KE_spectra_momz{OUTNC_suffix}.nc', decode_times=False, combine='by_coords').KE_spectra_momz
    ke_spectra_3dvel_tmp = ke_spectra_3dvel_tmp + fac * np.mean(ke_spectra_hvel[start_ind:,:,1:ncut] + ke_spectra_wvel[start_ind:,:,1:ncut], axis=0)

...
def get_pbl_analysis(topdir, exp_list):
  ...
  for exp_dir, exp_info in exp_list.items():
    ...
    sgs_heat_bnd_stab_flux[exp_dir] = -v1D_tb_list["SGS_HEAT_BC_STABFLX"].mean(["time"])
      
  return pt_list, eddy_momz_flux, sgs_momz_flux, mean_heat_flux, eddy_heat_flux, sgs_heat_flux, sgs_heat_bnd_stab_flux

def get_pbl_analysis_vel_vari(topdir, exp_list, mean_time=True):
  hwind_vari = {}
  vwind_vari = {}
  ...
  for exp_dir, exp_info in exp_list.items():
    ...
    print(f"time:{v1D_list.time.values}")
    print(f"time_s:{time_s}, time_e:{time_e}")
    if mean_time:
      hwind_vari[exp_dir] = v1D_list["HWindVari"].mean(["time"])
      vwind_vari[exp_dir] = v1D_list["VWindVari"].mean(["time"])
    else:
      hwind_vari[exp_dir] = v1D_list["HWindVari"]
      vwind_vari[exp_dir] = v1D_list["VWindVari"]

  return hwind_vari, vwind_vari

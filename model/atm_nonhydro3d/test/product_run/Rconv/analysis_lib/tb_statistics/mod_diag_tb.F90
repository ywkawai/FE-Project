...
module mod_diag_tb
  ...
  type(MeshField1D) :: SGS_HEAT_BC_STABFLX_V1D
  ...
  integer, parameter :: DIAGVID_TB_SGS_HEAT_BC_STABFLUX_V1D = 2
  integer, parameter :: DIAGVID_TB_SGS_MOMZ_EDDYFLUX_V1D   = 3
  integer, parameter :: DIAGVID_TB_Nu_V1D            = 4
  integer, parameter :: DIAGVID_TB_Kh_V1D            = 5
...
!OCL SERIAL
  subroutine diag_tb_Init( tb_scheme, mesh, meshV1D, &
    ...
    call SGS_HEAT_BC_STABFLX_V1D%Init("SGS_HEAT_BC_STABFLX", "J.m-2.s-1", meshV1D)
    ...
    if (ismaster) then
      ...
      call out_file_V1D%Def_Var(SGS_HEAT_BC_STABFLX_V1D, "horizontal averaged SGS BC stablization heat flux", &
        DIAGVID_TB_SGS_HEAT_BC_STABFLUX_V1D, DIMTYPEID_ZT, dtype, timeinv=out_tintrv )
  ...
  subroutine diag_tb_process( &
    ...
    do n=1, mesh%LOCAL_MESH_NUM
      ...
      call cal_auxvars( &
        ...
        SGS_HEAT_BC_STABFLX_V1D%local(n)%val,                    &
        Nu_V1D%local(n)%val,                                     &
        Kh_V1D%local(n)%val,                                     &
    ...
    if (ismaster) then
      ...
      call out_file_V1D%Write_var1D( DIAGVID_TB_SGS_HEAT_BC_STABFLUX_V1D, &
        SGS_HEAT_BC_STABFLX_V1D, start_time, end_time )
...
!OCL SERIAL
  subroutine diag_tb_Final()
    ...
    call SGS_HEAT_BC_STABFLX_V1D%Final()
...
!OCL SERIAL
  subroutine cal_auxvars( &
    ...
    real(RP), intent(out) :: SGS_HEAT_BND_STABFLX_V1D_(elem1D%Np,lmeshV1D%NeA)
    ...
    real(RP) :: SGS_HEAT_BND_STABFLX_z(elem3D%Np,lmesh%NeZ,lmesh%Ne2D)
    real(RP) :: SGS_HEAT_BND_STABFLX_reconst(elem3D%Np,lmesh%Ne)
    real(RP) :: del_flux_sgs_heat_bnd_stabflx(elem3D%NfpTot,lmesh%Ne)
    ...
    real(RP) :: nonormal_flux(elem3D%Nfp_v,lmesh%Ne2D)
    !--------------------------------------------------------------------
    ...
    if (domID==1) then
      ...
      do ke_z=1, lmesh%NeZ
        ...
        SGS_HEAT_BND_STABFLX_V1D_(:,ke_z) = 0.0_RP
        Nu_V1D_(:,ke_z) = 0.0_RP
        Kh_V1D_(:,ke_z) = 0.0_RP
      end do
      harea = 0.0_RP
    ...
    call calc_stab_bndflx( SGS_HEAT_BND_STABFLX_z, &
      PT_, DENS_hyd_, DDENS_, lmesh, elem3D, lmesh2D, lmesh2D%refElem2D )
    ...
    call get_del_flux_cent( del_flux_sgs_heat_bnd_stabflx, &
      SGS_HEAT_BND_STABFLX_z, lmesh, elem3D, 2, 2, nonormal_flux, nonormal_flux )
    call get_reconstructed_flux( SGS_HEAT_BND_STABFLX_reconst, & 
      SGS_HEAT_BND_STABFLX_z, del_flux_sgs_heat_bnd_stabflx, lmesh, elem3D )
    ...
    do ke_z=1, lmesh%NeZ
      ...
      do ke_x=1, lmesh%NeX
        ...
        do p=1, elem3D%Nnode_v
          ...
          SGS_HEAT_BND_STABFLX_V1D_(p,ke_z) = SGS_HEAT_BND_STABFLX_V1D_(p,ke_z)       &
                            + sum( int_w(:) * SGS_HEAT_BND_STABFLX_reconst(hSliceID(:),ke) )
    ...
    if ( domID == NLocalMeshPerPrc ) then
      ...
      do ke_z=1, lmesh%NeZ
        ...
        SGS_HEAT_BND_STABFLX_V1D_(:,ke_z) = CpDry * SGS_HEAT_BND_STABFLX_V1D_(:,ke_z) / harea

...
def get_var_list_z0(varname, dir_list, pe_num_x, pe_nun_y, pe_xs, pe_xe, pe_ys, pe_ye, zind):
  var_list = []
  for dir in dir_list:
    fpath = get_fpathlist(dir, "history", "","", pe_num_x, pe_nun_y, pe_xs, pe_xe, pe_ys, pe_ye)
    var = merge_xy(fpath)[varname].isel(z=zind)
    var_list.append(var)
  return var_list    
...
def gen_anim2_h( var_listlist, vmin_list, vmax_list, 
             anim_file, interval=90):
    
  var_num = len(var_listlist)
  X_list = []; Y_list = []
  fig, axes = plt.subplots(1, var_num, figsize=(8*var_num,9))
  
  for v in range(0,var_num):
    axes[v].set_xlabel("x [m]")
    axes[v].set_ylabel("y [m]")
    var_list = var_listlist[v]
    x, y = np.meshgrid(var_list[0].x.values, var_list[0].y.values)
  ...
  for k in range(0,len(var_list_1)):
    ...
    for time_ind in range(istart,len(time.values)):
        ...
        for v in range(0,var_num):
            var_list = var_listlist[v]
            cax = axes[v].pcolormesh(X_list[v], Y_list[v], var_list[k].isel(time=time_ind).values,
                vmin=vmin_list[v], vmax=vmax_list[v], 
                cmap = "jet"
            )
            cax_list.append(cax)

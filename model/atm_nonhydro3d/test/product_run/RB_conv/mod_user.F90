  ...
  subroutine USER_calc_tendency( this, atm )
    ...
    do n=1, atm%mesh%ptr_mesh%LOCAL_MESH_NUM
      ...
      call cal_tend_from_sfcflx( &
        ...
        atm%mesh%DOptrMat(3), atm%mesh%LiftOptrMat,                      & 
...
!OCL SERIAL  
  subroutine cal_tend_from_sfcflx( &
    ...
    DENS_hyd, PRES_hyd,                                  &
    PRES, PT, CPtot,                                     &
    Dz, Lift, lcmesh, elem, lcmesh2D, elem2D )
    ...
    real(RP) :: SFLX_SH_top(elem2D%Np,lcmesh2D%NeA)
    real(RP) :: SFLX_SH_btm(elem2D%Np,lcmesh2D%NeA)
    ...
    if ( cal_tend_flag ) then
      ...
      do ke=lcmesh%NeS, lcmesh%NeE
        ...
        RHOT_tp (:,ke) = RHOT_tp (:,ke) - LiftDelFlx(:) / CPtot(:,ke)
      end do
    end if
...
!OCL SERIAL  
  subroutine cal_del_flux( del_flux,      &
    ...
    !$omp parallel private(i, ke, ke2D, p)
    !$omp workshare
    del_flux(:,:) = 0.0_RP
    !$omp end workshare
    ...
    do p=1, elem2D%Np
      ...
      del_flux(i,1) = sflx_sh_btm(p,ke2D) * nz(i)
    end do
    end do
    !$omp do collapse(2)
    do ke2D=1, lmesh2D%Ne
    do p=1, elem2D%Np
      ...
      del_flux(i,1) = sflx_sh_top(p,ke2D) * nz(i)
    end do
    end do
    !$omp end do
    !$omp end parallel
...
!OCL SERIAL  
  subroutine exp_SetInitCond_RB_convection( this,                   &
    ...
    case ( "BUBBLE" )
      !$omp parallel do private( vx, vy, vz, x_intrp, y_intrp, z_intrp, PT )
      do ke=lcmesh%NeS, lcmesh%NeE
    ...
    case ( "RANDOM" )
      !$omp parallel do private( ke, ke2D, rndm, EXNER ) collapse(3)
      ...
      do ke_x=1, lcmesh%NeX
        ...
        if ( ke_z==1 ) bnd_SFC_PRES(:,ke2D) = PRES00
      end do
      end do
      end do

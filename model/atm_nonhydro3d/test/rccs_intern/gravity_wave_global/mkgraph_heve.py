...
#----
# run_dir = "Eh8Ez6P7_heve"
# dist_dir = f"figs/{run_dir}"
#---
run_dir = "Eh16Ez12P3_heve"
dist_dir = f"figs/{run_dir}"
...
levels = np.arange(-3e-6, 3e-6+3e-7, 3e-7)    
plot_var_xz(vars["Umet"].sel(lat=0).isel(time=-1)[:,0,:], 'Zonal wind [m/s]', -3e-6, 3e-6, levels,  f"{dist_dir}/Umet.png")

levels = np.arange(-1e-4, 1e-4+1e-5, 1e-5)    
plot_var_xz(vars["W"].sel(lat=0).isel(time=-1)[:,0,:], 'Vertical wind [m/s]', -1e-4, 1e-4, levels,  f"{dist_dir}/W.png")

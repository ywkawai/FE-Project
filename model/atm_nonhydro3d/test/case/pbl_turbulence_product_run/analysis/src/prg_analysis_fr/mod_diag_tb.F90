  ...
  subroutine cal_auxvars( &
    ...
    do ke_z=1, lmesh%NeZ
      ...
      do ke_x=1, lmesh%NeX
        ...
        SGS_HEAT_EDDYFLX_z(:,ke_z,ke2D) = DENS_lc(:) * Kh_(:,ke) * dPTdz_(:,ke)
        SGS_MOMZ_EDDYFLX_z(:,ke_z,ke2D) = DENS_lc(:) &
          * (   2.0_RP * Nu_(:,ke) * ( 2.0_RP * S33_(:,ke) - S11_(:,ke) - S22_(:,ke) ) / 3.0_RP &
              - TKE_(:,ke) * 2.0_RP/3.0_RP                                                      )        
      end do    
      end do  
    end do    

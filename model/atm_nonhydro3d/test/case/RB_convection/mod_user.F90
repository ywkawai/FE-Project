...
!OCL SERIAL  
  subroutine cal_tend_from_sfcflx( &
    ...
    real(RP) :: del_flux_grad(elem%NfpTot,lcmesh%Ne)
    ...
    if ( cal_tend_flag ) then
      ...
      call cal_del_flux( del_flux, del_flux_grad, &
        SFLX_MOMX_btm, SFLX_MOMX_top, SFLX_MOMY_btm, SFLX_MOMY_top, &
        SFLX_SH_btm, SFLX_SH_top, lcmesh%normal_fn(:,:,3), &
        PT, &
        lcmesh%VMapM, lcmesh%VMapP, lcmesh, elem, lcmesh2D, elem2D )

      !$omp parallel do private(ke2D, ke_top, ke_btm, Fz, LiftDelFlx)
      do ke2D=lcmesh2D%NeS, lcmesh2D%NeE
        ...
        call sparsemat_matmul( Lift, lcmesh%Fscale(:,ke_btm)*del_flux_grad(:,ke_btm), LiftDelFlx )
        DzPT(:,ke_btm) = lcmesh%Escale(:,ke_btm,3,3) * Fz(:) + LiftDelFlx(:)

        call sparsemat_matmul( Dz, PT(:,ke_top), Fz)
        call sparsemat_matmul( Lift, lcmesh%Fscale(:,ke_top)*del_flux_grad(:,ke_top), LiftDelFlx )
      ...
      do ke2D=lcmesh2D%NeS, lcmesh2D%NeE
        ...
        call sparsemat_matmul( Sz, ( DENS_hyd(:,ke_btm) + DDENS(:,ke_btm) ) * Kh_btm(:,ke2D) * DzPT(:,ke_btm), Fz)
        ...
        !-
        call sparsemat_matmul( Sz, ( DENS_hyd(:,ke_top) + DDENS(:,ke_top) ) * Kh_top(:,ke2D) * DzPT(:,ke_top), Fz)
...
!OCL SERIAL  
  subroutine cal_del_flux( del_flux, del_flux_grad,     &
    ...
    real(RP), intent(out) ::  del_flux_grad(elem%NfpTot*lmesh%Ne)
    ...
    !$omp parallel private(i, i2, iP, iM, ke, ke2D, p)
    !$omp workshare
    del_flux(:,:) = 0.0_RP
    ...
    do p=1, elem2D%Np
      ...
      del_flux(i,1) = sflx_momx_btm(p,ke2D) * nz(i)
      del_flux(i,2) = sflx_momy_btm(p,ke2D) * nz(i)
      del_flux(i,3) = sflx_sh_btm(p,ke2D) * nz(i)

      iM = vmapM(i2); iP = vmapP(i2)
      del_flux_grad(i2) = 0.5_RP * ( PT(iP) - PT(iM) ) * nz(i2)
    end do
    end do
    !$omp do collapse(2)
    do ke2D=1, lmesh2D%Ne
    do p=1, elem2D%Np
      ...
      i = (ke-1)*elem%NfpTot &
        + elem%Nfaces_h*elem%Nfp_h + elem%Nfp_v + p

      i2 = (ke-1)*elem%NfpTot &
         + elem%Nfaces_h*elem%Nfp_h + p
      
      del_flux(i,1) = sflx_momx_top(p,ke2D) * nz(i)
      del_flux(i,2) = sflx_momy_top(p,ke2D) * nz(i)

...
!!
!! @par Description
!!          User defined module for a test case of boundary layer scheme
!!
...
!OCL SERIAL
  subroutine USER_mkinit ( this, atm )
    ...
    call exp_manager%Init('boundary_layer')
    call exp_manager%Regist_SetInitCond( exp_SetInitCond_boundary_layer )
...
!OCL SERIAL  
  subroutine exp_SetInitCond_boundary_layer( this, &
    ...
    real(RP) :: RHOT_hyd(elem%Np,lcmesh%Ne)
    ...
    if( ierr < 0 ) then !--- missing
       LOG_INFO("BOUNDARY_LAYER_setup",*) 'Not found namelist. Default used.'
    elseif( ierr > 0 ) then !--- fatal error
       LOG_ERROR("BOUNDARY_LAYER_setup",*) 'Not appropriate names in namelist PARAM_EXP. Check!'
       call PRC_abort
    endif
    LOG_NML(PARAM_EXP)
    ...
    if ( ENV_RH > 0.0_RP ) then
      LOG_INFO("BOUNDARY_LAYER_setup",*) 'Calculate QV from RH'
      LOG_INFO("BOUNDARY_LAYER_setup",*) 'ENV_RH = ', ENV_RH, ' [%]'
    ...
    do ke_z=1, lcmesh%NeZ
      ...
      ! MOMX(:,ke) = ( DENS_hyd(:,ke) + DDENS(:,ke) ) * ( U0 + VSHEAR_HVEL * z(:,ke) )
      MOMX(:,ke) = ( DENS_hyd(:,ke) + DDENS(:,ke) ) * ( U0 + 2.0_RP * cos( PI * z(:,ke) / ( dom_zmax - dom_zmin ) ) )
      MOMY(:,ke) = 0.0_RP
      MOMZ(:,ke) = 0.0_RP
    end do
    end do
    ...
    return
  end subroutine exp_SetInitCond_boundary_layer

...
contains
  ...
  subroutine exp_SetInitCond_inertia_gravity_wave( this,                      &
    ...
    do k=1, lcmesh%Ne
      ...
      DENS_hyd(:,k) = matmul( IntrpMat, PRES00 / (Rdry * THETA_hyd_intrp(:)) * (EXNER_hyd_intrp(:))**(CvDry/Rdry) )

      DTHETA_intrp(:) = DTHETA * sin( PI*z_intrp(:)/10.0E3_RP) &
                        / ( 1.0_RP + ((x_intrp(:) - x_c)/r_x)**2 + ((y_intrp(:) - y_c)/r_y)**2 )

      DDENS(:,k) =  matmul(IntrpMat, PRES00 / (Rdry * (THETA_hyd_intrp(:) + DTHETA_intrp(:))) * (EXNER_hyd_intrp(:))**(CvDry/Rdry))  &
                  - DENS_hyd(:,k)

...
REGRID_TOPO_INI_DIR="./ini_topo_regrid"
#----------------------
...
&PARAM_MKTOPO_INPUT_FILE
  IN_BASENAME = "{REGRID_TOPO_INI_DIR}/TOPO", 
  VARNAME     = "topo", 
/
#** ATMOS ******************************************************
&PARAM_ATMOS
...
&PARAM_ATMOS_MESH
  ...
  PolyOrder_v      = {porder},
  FZ               = {fz}, 
  TOPO_IN_BASENAME  = '{in_topo_basename}',    
  LumpedMassMatFlag = .true.,   
/
&PARAM_ATMOS_VARS
  CHECK_RANGE = .true. ,
...
&PARAM_REGRID_MESH
 ...
 out_MeshType = "CUBEDSPHERE2D", 
/
&PARAM_REGRID_INTERP_FIELD
  !- input --------------------
  in_basename="{in_topo_basename}",     
  vars = "topo",  
/
&PARAM_REGRID_FILE
  !-- output ----------------
  out_basename="{REGRID_TOPO_INI_DIR}/TOPO", 
...
/
&PARAM_REGRID_OUTMESH2D_CUBEDSPHERE
  NLocalMeshPerPrc = 1, 
  Nprc             = {nprc}, 
  NeGX             = {neh},
  NeGY             = {neh},
...
def mksh_job_run( conf_path, job_name, 
  ...
  if spinup1_flag:
    init_cmd = f"""mkdir -p {REGRID_TOPO_INI_DIR}/
mpiexec -np {nprc} -stdout-proc ./output.%j/%/1000r/stdout -stderr-proc ./output.%j/%/1000r/stderr \\
  ${{SCALE_DG_REGRID_BIN}} regrid_ini_topo.conf || exit 1    
...
mpiexec -np {nprc} -stdout-proc ./output.%j/%/1000r/stdout -stderr-proc ./output.%j/%/1000r/stderr \\
  ...
  jobshell_s = f"""
llio_transfer ${{SCALE_DG_REGRID_BIN}} ${{SCALE_DG_INIT_BIN}} ${{SCALE_DG_BIN}} ${{SCALE_DG_REGRID_BIN}} *.conf

{init_cmd}
mpiexec -np {nprc} -stdout-proc ./output.%j/%/1000r/stdout -stderr-proc ./output.%j/%/1000r/stderr \\
  ${{SCALE_DG_BIN}} run.conf || exit 1  

llio_transfer --purge ${{SCALE_DG_REGRID_BIN}} ${{SCALE_DG_INIT_BIN}} ${{SCALE_DG_BIN}} *.conf    
  """
...
def mk_conf_sh( exp_name, exp_info ):
    ...
    in_topo_ori_basename = exp_info["in_topo_ori_basename"]
    in_topo_ori_nprc = exp_info["in_topo_ori_nprc"]
    in_topo_ori_eh = exp_info["in_topo_ori_eh"]
    in_topo_ori_porder = exp_info["in_topo_ori_porder"]

    out_dir_pref=f"./{exp_name}"
    print(out_dir_pref)
    
    os.makedirs(out_dir_pref+"/spinup1", exist_ok=True)
    mkconf_regrid_ini_topo(f"{out_dir_pref}/spinup1/regrid_ini_topo.conf",
                in_topo_ori_basename, in_topo_ori_nprc, in_topo_ori_eh, in_topo_ori_porder, 
                nprc, eh, porder ) 
    ...
    mkconf_run(f"{out_dir_pref}/spinup1/run.conf", 
               "init_00000101-000000.000", 1, 1, 1, 50, 
                ...
                exp_info["mf_alph_ini"], exp_info["mf_ordh"], exp_info["mf_alpv_ini"], exp_info["mf_ordv"], 
                "../spinup1/TOPO", 
                True) 
    ...
    for runno in range(1, spinup_run_num+1):
      ...
      mksh_job_run(f"{out_dir_pref}/spinup{runno}/job_run.sh", f"HST_Eh{eh}P{porder}_sp{runno}", 
    ...
    for runno in range(1,int(1000/day_per_run)+1):
      ...
      mksh_job_run(f"{out_dir_pref}/run{runno}/job_run.sh", f"HST_Eh{eh}P{porder}_run{runno}", 

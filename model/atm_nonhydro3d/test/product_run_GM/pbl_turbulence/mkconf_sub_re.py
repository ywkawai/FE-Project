...
def mkconf_regrid_init( conf_path, 
  ...
  conf_regrid_s = f"""#--- Configuration file for PBL turbulence  -------
&PARAM_IO
...
def mk_conf_sh( exp_name, exp_info ):
    ...
    ini_regrid_dir = f"{out_dir_pref0}/run{run_s-1}"
    os.makedirs(ini_regrid_dir, exist_ok=True)
    mkconf_regrid_init(f"{ini_regrid_dir}/regrid_restart.conf", 
                exp_info["rg_in_basename"], exp_info["rg_in_nprc"], exp_info["rg_in_Eh"], exp_info["rg_in_Ez"], exp_info["rg_in_porder"], exp_info["rg_in_fz"],                 
                exp_info["rg_out_basename"], nprc, eh, ez, porder, fz, shallow_atm_approx )

    mksh_job_regrid(f"{ini_regrid_dir}/job_regrid_restart.sh", f"REGRS_E{eh}P{porder}", "regrid_restart.conf", 
                nprc, exp_info["regrid_elapse_time"], 
                "")
        
    #--
    date_time0 = datetime.datetime(1, 1, 1, 0, 0, 0, 0) + datetime.timedelta(hours=exp_info["ini_hour"])
    hr_per_run = exp_info["hour_per_run"]
    
    for runno_ in range(1,int( exp_info["integ_hour"] / hr_per_run)+1):
      runno = run_s + runno_ - 1
      date_time = date_time0 + datetime.timedelta(hours=hr_per_run*(runno_-1))

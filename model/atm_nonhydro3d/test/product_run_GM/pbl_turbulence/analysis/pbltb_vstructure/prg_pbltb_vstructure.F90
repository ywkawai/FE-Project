  ...
  subroutine initialize()
    ...
    if (is_spec_FZ) then
      call mesh3D%Init( NeGX, NeGY, NeZ, RPlanet, dom_zmin, dom_zmax, refElem3D, NLocalMeshPerPrc, &
        nproc=Nprc, FZ=FZ(1:NeZ+1), shallow_approx=SHALLOW_ATM_APPROX_FLAG )
    else
      call mesh3D%Init( NeGX, NeGY, NeZ, RPlanet, dom_zmin, dom_zmax, refElem3D, NLocalMeshPerPrc, &
        nproc=Nprc, shallow_approx=SHALLOW_ATM_APPROX_FLAG )
    end if
    ...
    ! Data
    LOG_INFO("PBLTB_VSTRUCTURE",*) 'Setup variables..'
    ...
    ! Communication
    call vars_comm%Init( 7, 1, 0, mesh3D )
    comm_vars_ptr(1)%field3d => DDENS
    comm_vars_ptr(2)%field3d => MOMZ
    comm_vars_ptr(3)%field3d => DRHOT
    comm_vars_ptr(4)%field3d => DENS_hyd
    comm_vars_ptr(5)%field3d => PRES_hyd
    comm_vars_ptr(6)%field3d => PT
    comm_vars_ptr(7)%field3d => PRES
    comm_vars_ptr(8)%field3d => MOMX
    comm_vars_ptr(9)%field3d => MOMY
...
end program prg_pbltb_vstructure
...
"""
  ...
  conf_run_s = f"""#--- Configuration file for a test case of two-dimensional linear advection with deformation flow -------
&PARAM_RESTART
...
SCALE_DG_INIT_BIN={SCALE_DG_BIN_PATH}/scale-dg_init_gm_linadv_deform_flow
SCALE_DG_BIN={SCALE_DG_BIN_PATH}/scale-dg_gm_linadv_deform_flow
...
def mk_conf_jobsh( exp_name, exp_info, ptracer_shape, exp_dir_postfix="" ):
  nprc = exp_info["nprc"]
  eh = exp_info["Eh"]
  ez = exp_info["Ez"]
  ...
  mkconf_init(f"{out_dir}/init.conf", 
              nprc, eh, ez, porder, 
              ptracer_shape, exp_info["initgp_porder"] )
  
  mkconf_run(f"{out_dir}/run.conf", 
              nprc, eh, ez, porder, 
              ptracer_shape,exp_info["initgp_porder"], 
              exp_info["ATMOS_dt"], exp_info["dt"], 

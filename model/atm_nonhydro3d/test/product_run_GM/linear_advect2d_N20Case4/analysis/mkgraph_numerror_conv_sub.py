...
def mkgraph_num_convergence(data_list, slope_dof_list, slope_list, ylabel_list, ylim_list, outpng_path):
  ...
  for i in range(0,data_num):  
    for key, data in data_list[i].items():
      numerror = data
      color, lstyle, lbl = get_color_and_linestyle_and_label(key)
      ax_list[i].plot( data.DOF, data, 
        label=f"${lbl}$", 
        color=color, 
        linestyle=lstyle, 
        marker="s", markeredgecolor="k", markersize=10 )
...
def get_numerror_data_list(dir, exp_list):
  ...
  for subdir in exp_list:
    list_numerror[subdir] = get_numerror_data(f"{dir}/{subdir}/analysis/NUMERROR_LOG.peall")

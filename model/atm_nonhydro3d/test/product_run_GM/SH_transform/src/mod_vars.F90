...
!OCL SERIAL
  subroutine vars_init( &
    vars, levels, level_units, Np2D, Ne2D, Mt, mesh3D_list, target_proc_s, &
    ...
    allocate( g_var2D(varNum,LevelNum,Np2D,Ne2D,mesh_num) )
    allocate( s_var2D(varNum,LevelNum,0:Mt,0:Mt,2) )
    ...
    if (output_flag) then
      ...
      call nc_check( nf90_def_dim( ncido, 'level', LevelNum, level_dim_id ) )
      ...
      call nc_check( nf90_def_var( ncido, 'level', NF90_DOUBLE, level_dim_id, level_id ) )
      ...
      call nc_check( nf90_put_att( ncido, level_id, 'units', trim(level_units) ) )
      ...
      call nc_check( nf90_put_var(ncido, level_id, levels ) )
    end if
...
!OCL SERIAL
  ...
  subroutine vars_write( istep, Mt, LevelNum )
    ...
    integer :: vid
    real(RP) :: s_var2D_r_buf(Mt+1,Mt+1,LevelNum)
    real(RP) :: s_var2D_i_buf(Mt+1,Mt+1,LevelNum)
    ...
    if (output_flag) then
      ...
      do vid=1, varNum
        do k=1, LevelNum
          s_var2D_r_buf(1:Mt+1,1:Mt+1,k) = s_var2D(vid,k,0:Mt,0:Mt,1)
          s_var2D_i_buf(1:Mt+1,1:Mt+1,k) = s_var2D(vid,k,0:Mt,0:Mt,2)
        end do
        call nc_check( nf90_put_var( ncido, var_id_list_r(vid), s_var2D_r_buf, &
          start=[1,1,1,istep], count=[Mt+1,Mt+1,LevelNum,1]) )
        call nc_check( nf90_put_var( ncido, var_id_list_i(vid), s_var2D_i_buf, &
          start=[1,1,1,istep], count=[Mt+1,Mt+1,LevelNum,1]) )
      end do
    end if
...
!OCL SERIAL
  subroutine vars_read( istep, mesh3D_list, levels )
    ...
    do m=1, size(mesh3D_list)
        ...
        do ke = lcmesh3D%NeS, lcmesh3D%NeE
            ...
            do ph=1, elem3D%Nnode_h1D**2
              ...
              if ( lcmesh3D%pos_en(p,ke,3) <= levels(k)   &
                ...
                g_var2D(vid,k,ph,ke2D,m) = tmp_field3D%local(1)%val(p,ke)
              end if
            end do
            end do
          end do
      ...
      gam(:,:) = 1.0_RP
      do k = 1, LayerNum
        tmp_U(:,:) = g_var2D(vid_U,k,:,:,m)
        tmp_V(:,:) = g_var2D(vid_V,k,:,:,m)
        ...
        g_var2D(vid_U,k,:,:,m) = tmp_Umet(:,:)
        g_var2D(vid_V,k,:,:,m) = tmp_Vmet(:,:)
      end do

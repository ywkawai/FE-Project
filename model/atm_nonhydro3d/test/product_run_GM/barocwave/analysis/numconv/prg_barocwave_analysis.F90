#include "scalelib.h"
program prg_barocwave_analysis
  use mpi
  use scale_precision
  use scale_io
  use scale_prof
  ...
  type(MeshFieldContainer) :: comm_vars_ptr(7)
  ...
  character(*), parameter :: dtype = 'REAL4'

  real(RP) :: start_time, end_time

  type(MeshFieldAnalysisNumerror2D) :: numerror_analysis_sfc
  type(MeshFieldAnalysisNumerror3D) :: numerror_analysis
  integer :: numerror_vid(5)
  integer :: numerror_sfc_vid(1)
...
!    call in_file%Read_Var(DIMTYPEID_XYZT, "PT", PT, step=istep)
    ...
    do n=1, mesh3D%LOCAL_MESH_NUM
      ...
      call set_pres( PRES%local(n)%val, &
        DRHOT%local(n)%val, PRES_hyd%local(n)%val, lcmesh3D, lcmesh3D%refElem3D )
      call set_pres( PRES_ref%local(n)%val, &
        DRHOT_ref%local(n)%val, PRES_hyd_ref%local(n)%val, lcmesh3D, lcmesh3D%refElem3D )

      ! call set_prgvars( &
      !   MOMX%local(n)%val, MOMY%local(n)%val, MOMZ%local(n)%val,                             & ! (out)
      !   PRES%local(n)%val,                                                                   & ! (out)
      !   DDENS%local(n)%val, U%local(n)%val, V%local(n)%val, W%local(n)%val, DRHOT%local(n)%val, & ! (in)
    ...
    call numerror_analysis_sfc%Evaluate( &
      istep, end_time, mesh3D%mesh2D, evaluate_error_sfc_lc )
...
!OCL SERIAL
  subroutine set_pres( PRES_, &
    ...
    do ke=lmesh%NeS, lmesh%NeE
      ...
      PRES_(:,ke) = PRES00 * ( Rdry / PRES00 * ( RHOT_hyd(:) + THERM_(:,ke) ) )**(CPdry/CVdry)
    end do

    return
  end subroutine set_pres
!OCL SERIAL
  subroutine set_prgvars( MOMX_, MOMY_, MOMZ_, PRES_,  &
    DDENS_, U_, V_, W_, THERM_, DENS_hyd_, PRES_hyd_,  &
    lmesh, elem )
...
!OCL SERIAL
  subroutine evaluate_error_lc( this, q, qexact, qexact_intrp, lcmesh, elem3D, intrp_epos )
    ...
    ! do v=1, 3
    ! do ke=lcmesh%NeS, lcmesh%NeE
    !   where ( lcmesh%pos_en(:,ke,1) < window_xs .or. lcmesh%pos_en(:,ke,1) > window_xe .or. &
...
!OCL SERIAL
  subroutine evaluate_error_sfc_lc( this, q, qexact, qexact_intrp, lcmesh, elem2D, intrp_epos )
    use scale_localmeshfield_base, only: LocalMeshFieldBase
    implicit none
    ...
    !$omp parallel private(vid, ke2D)
    !$omp do
    do ke2D=lcmesh%NeS, lcmesh%NeE
      vid = numerror_sfc_vid(1)
      q(:,ke2D,vid) = PRES%local(n)%val(refElem3D%Hslice(:,1),ke2D)
      qexact(:,ke2D,vid) = PRES_ref%local(n)%val(refElem3D%Hslice(:,1),ke2D)
    end do
    !$omp end do
    ...
    do vid=1, this%var_num
    do ke2D=lcmesh%NeS, lcmesh%NeE      
      qexact_intrp(:,ke2D,vid) = matmul( this%IntrpMat, qexact(:,ke2D,vid) )
    end do
    end do
    !$omp end do
    !$omp end parallel
    ...
    return
  end subroutine evaluate_error_sfc_lc

!OCL SERIAL
  subroutine calc_diagvars( VOR_, &
    DDENS_, U_, V_, W_, DRHOT_,      &
    u1_, u2_, DENS_hyd_, PRES_hyd_,  &
    lmesh, elem )
    class(LocalMesh3D), intent(in) :: lmesh
    class(ElementBase3D), intent(in) :: elem
    real(RP), intent(out) :: VOR_(elem%Np,lmesh%NeA)
    real(RP), intent(in) :: DDENS_(elem%Np,lmesh%NeA)
    real(RP), intent(in) :: U_(elem%Np,lmesh%NeA)
    real(RP), intent(in) :: V_(elem%Np,lmesh%NeA)
    real(RP), intent(in) :: W_(elem%Np,lmesh%NeA)
    ...
    real(RP) :: del_flux(elem%NfpTot,lmesh%Ne,1)
    !---------------------------------------------

    call cal_del_flux( del_flux,         & ! (out)
      U_, V_, u1_, u2_,                      & ! (in)
      lmesh%Gsqrt, lmesh%normal_fn(:,:,1), lmesh%normal_fn(:,:,2), & ! (in)
      lmesh%vmapM, lmesh%vmapP, lmesh, elem                        )                                          ! (in)

    !$omp parallel do private(ke, DENS, Fx, Fy, Fz, LiftDelFlx)
    do ke = lmesh%NeS, lmesh%NeE
...
!OCL SERIAL
  subroutine cal_del_flux( del_flux,           &
    ...
    class(LocalMesh3D), intent(in) :: lmesh
    class(ElementBase3D), intent(in) :: elem  
    real(RP), intent(out) ::  del_flux(elem%NfpTot,lmesh%Ne,1)
    real(RP), intent(in) ::  U_(elem%Np*lmesh%NeA)  
    real(RP), intent(in) ::  V_(elem%Np*lmesh%NeA)  
    real(RP), intent(in) ::  u1_(elem%Np*lmesh%NeA)  
    real(RP), intent(in) ::  u2_(elem%Np*lmesh%NeA)
...
!OCL SERIAL
  subroutine initialize()
    ...
    character(len=H_MID) :: NUMERROR_SFC_LOG_OUT_BASENAME = 'LOG_NUMERROR_SFC'

    !-
    namelist / PARAM_BAROCWAVE_ANALYSIS / &
      in_filebase,                    &
      in_bs_filebase,                 &
      in_ref_filebase,                &
      in_bs_ref_filebase,             &
    ...
    rewind(IO_FID_CONF)
    read(IO_FID_CONF,nml=PARAM_BAROCWAVE_ANALYSIS,iostat=ierr)
    if( ierr < 0 ) then !--- missing
        LOG_INFO("BAROCWAVE_ANALYSIS_setup",*) 'Not found namelist. Default used.'
    elseif( ierr > 0 ) then !--- fatal error
        LOG_ERROR("BAROCWAVE_ANALYSIS_setup",*) 'Not appropriate names in namelist PARAM_BAROCWAVE_ANALYSIS. Check!'
        call PRC_abort
    endif
    LOG_NML(PARAM_BAROCWAVE_ANALYSIS)
    ...
    if( ierr < 0 ) then !--- missing
        LOG_INFO("BAROCWAVE_ANALYSIS_setup",*) 'Not found namelist. Default used.'
    elseif( ierr > 0 ) then !--- fatal error
        LOG_ERROR("BAROCWAVE_ANALYSIS_setup",*) 'Not appropriate names in namelist PARAM_ATM_MESH. Check!'
        call PRC_abort
    endif
    LOG_NML(PARAM_ATMOS_MESH)

    !- Mesh
    LOG_INFO("BAROCWAVE_ANALYSIS",*) 'Setup mesh..'
    ...
    call PRES_ref%Init("PRES", "Pa", mesh3D)
...
!        dtype, fileexist, myrank=myrank )
    ...
    call numerror_analysis_sfc%Init( &
      PolyOrderErrorCheck, NUMERROR_SFC_LOG_OUT_BASENAME, 1, mesh3D%refElem2D )

    call numerror_analysis_sfc%Regist( "SFCPRES", "Pa", numerror_sfc_vid(1) )

    ! Communication
    call vars_comm%Init( 5, 1, 0, mesh3D )
    comm_vars_ptr(1)%field3d => DDENS
    comm_vars_ptr(2)%field3d => W
    comm_vars_ptr(3)%field3d => DRHOT
    comm_vars_ptr(4)%field3d => DENS_hyd
    comm_vars_ptr(5)%field3d => PRES_hyd
    comm_vars_ptr(6)%field3d => U
    comm_vars_ptr(7)%field3d => V
    call vars_comm%SetCovariantVec(1, u1, u2)
...
!OCL SERIAL
  subroutine finalize()
    ...
    call numerror_analysis_sfc%Final()
...
end program prg_barocwave_analysis

...
!OCL SERIAL
  subroutine initialize()
    ...
    !-
    namelist / PARAM_GRAVWAVE_ANALYSIS / &
      in_filebase,                    &
      in_bs_filebase,                 &
      in_ref_filebase,                &
      in_bs_ref_filebase,             &
    ...
    rewind(IO_FID_CONF)
    read(IO_FID_CONF,nml=PARAM_GRAVWAVE_ANALYSIS,iostat=ierr)
    if( ierr < 0 ) then !--- missing
        LOG_INFO("GRAVWAVE_ANALYSIS_setup",*) 'Not found namelist. Default used.'
    elseif( ierr > 0 ) then !--- fatal error
        LOG_ERROR("GRAVWAVE_ANALYSIS_setup",*) 'Not appropriate names in namelist PARAM_GRAVWAVE_ANALYSIS. Check!'
        call PRC_abort
    endif
    LOG_NML(PARAM_GRAVWAVE_ANALYSIS)
    ...
    if( ierr < 0 ) then !--- missing
        LOG_INFO("GRAVWAVE_ANALYSIS_setup",*) 'Not found namelist. Default used.'
    elseif( ierr > 0 ) then !--- fatal error
        LOG_ERROR("GRAVWAVE_ANALYSIS_setup",*) 'Not appropriate names in namelist PARAM_ATM_MESH. Check!'
        call PRC_abort
    endif
    LOG_NML(PARAM_ATMOS_MESH)

    !- Mesh
    LOG_INFO("GRAVWAVE_ANALYSIS",*) 'Setup mesh..'
    ...
    ! call comm2D%Init( 1, 0, mesh3D%mesh2D )
    ! call comm3D%Init( 1, 1, mesh3D )
    ...
    ! Data
    LOG_INFO("GRAVWAVE_ANALYSIS",*) 'Setup variables..'
...
end program prg_gravwave_analysis

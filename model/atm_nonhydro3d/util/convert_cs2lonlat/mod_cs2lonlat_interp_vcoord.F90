...
module mod_cs2lonlat_interp_vcoord
  !-----------------------------------------------------------------------------
  !
  !++ used modules
  !
...
!OCL SERIAL
  subroutine interp_vcoord_Init( this, out_mesh3D, nodeMap_list )
    ...
    use mod_cs2lonlat_interp_mesh, only: &
      in_NeGX, in_NeGY, in_NeGZ, &
      in_NprcX_lc, in_NprcY_lc
    implicit none
    ...
    do n=1, out_mesh3D%LOCAL_MESH_NUM
      ...
      call interp_vcoord_search_pos_model( this, &
         ...
         nmap%in_mesh3D_list, in_NeGZ, in_NeGX/in_NprcX_lc, in_NeGY/in_NprcY_lc     ) ! (in)
    end do
    ...
    if ( this%vintrp_typeid /= INTERP_VCOORD_MODEL_ID ) then  
      ...
      call this%out_mesh%SetDimInfo( MeshBase3D_DIMTYPEID_X, 'lon', 'degree_east', 'longitude' )
      call this%out_mesh%SetDimInfo( MeshBase3D_DIMTYPEID_Y, 'lat', 'degree_north', 'latitude' )
      call this%out_mesh%SetDimInfo( MeshBase3D_DIMTYPEID_XYZ, 'lonlatv', 'degree', 'longitude,latitude,altitude' )
...
!OCL SERIAL
  subroutine interp_vcoord_search_pos_model( this, elem_k, elem_out, &
    ...
    in_mesh3D_list, in_NeGZ, in_NeX, in_NeY                          )
    ...
    type(MeshCubedSphereDom3D), intent(in), target :: in_mesh3D_list(:)
    ...
    do p_h=1, elem3D%Nnode_h1D**2
      ...
      if ( in_n > 0 .and. in_prc > 0 .and.                 &
        ...
        in_lcmesh => in_mesh3D_list(in_prc)%lcmesh_list(in_n)
        do ke_z=1, lcmesh%NeZ

...
contains
  subroutine ATMOS_DYN_driver_setup()
    ...
    !
    call atm_dyn_nonhydro3d_Init( mesh )
    ...
    if (ATMOS_DYN_EXPFILTER_FLAG) then
      call atm_dyn_nonhydro3d_prepair_expfilter( mesh%lcmesh_list(1)%refElem3D,         &
  ...
  subroutine ATMOS_DYN_driver()   
    ...
    use mod_atmos_vars, only: &
      ...
      VARS_DDENS_ID, VARS_MOMX_ID, VARS_MOMY_ID, VARS_MOMZ_ID, VARS_DRHOT_ID,          &
      VARS_GxU_ID, VARS_GyU_ID, VARS_GzU_ID, VARS_GxV_ID, VARS_GyV_ID, VARS_GzV_ID,    &
      VARS_GxW_ID, VARS_GyW_ID, VARS_GzW_ID, VARS_GxPT_ID, VARS_GyPT_ID, VARS_GzPT_ID, &
    ...
    do rkstage=1, tinteg_lc(1)%nstage
      ...
      if (ATMOS_DYN_CALC_DIFFVARS_FLAG) then
        ...
        do n=1, mesh%LOCAL_MESH_NUM
          ...
          call atm_dyn_nonhydro3d_cal_grad_diffVars( &
            GxU%local(n)%val,  GyU%local(n)%val,  GzU%local(n)%val,                                           & ! (out)
            GxV%local(n)%val,  GyV%local(n)%val,  GzV%local(n)%val,                                           & ! (out)
            GxW%local(n)%val,  GyW%local(n)%val,  GzW%local(n)%val,                                           & ! (out)
            GxPT%local(n)%val, GyPT%local(n)%val, GzPT%local(n)%val,                                          & ! (out)
            DDENS%local(n)%val, MOMX%local(n)%val, MOMY%local(n)%val, MOMZ%local(n)%val, DRHOT%local(n)%val,  & ! (in)
      ...
      do n=1, mesh%LOCAL_MESH_NUM
        ...
        call atm_dyn_nonhydro3d_cal_tend( &
          ...
          tinteg_lc(n)%tend_buf2D(:,:,VARS_MOMY_ID,tintbuf_ind),                                            &
          ...
          GxU%local(n)%val,  GyU%local(n)%val,  GzU%local(n)%val,                                           &
          GxV%local(n)%val,  GyV%local(n)%val,  GzV%local(n)%val,                                           &
          GxW%local(n)%val,  GyW%local(n)%val,  GzW%local(n)%val,                                           &
          GxPT%local(n)%val, GyPT%local(n)%val, GzPT%local(n)%val,                                          & 
          ATMOS_DYN_VISCCOEF_H, ATMOS_DYN_VISCCOEF_V,                                                       &
          ATMOS_DYN_DIFFCOEF_H, ATMOS_DYN_DIFFCOEF_V,                                                       &          
          Dx, Dy, Dz, Sx, Sy, Sz, Lift, lcmesh, lcmesh%refElem3D ) 
        ...
        call tinteg_lc(n)%Advance( rkstage, MOMY%local(n)%val, VARS_MOMY_ID,       &
                                   1, lcmesh%refElem%Np, lcmesh%NeS, lcmesh%NeE )  
        ...
        if (ATMOS_DYN_EXPFILTER_FLAG) then
          ...
          call atm_dyn_nonhydro3d_filter_prgvar( &
            DDENS%local(n)%val, MOMX%local(n)%val, MOMY%local(n)%val, MOMZ%local(n)%val, DRHOT%local(n)%val, &

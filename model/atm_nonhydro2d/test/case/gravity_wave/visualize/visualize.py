...
def set_fig_XY_axis(ax):
  ...
  ax.yaxis.set_major_locator(tick.MultipleLocator(5e3))
  ax.yaxis.set_minor_locator(tick.MultipleLocator(1e3))
...
pcm = ax.pcolormesh(X, Z, dtheta.isel(time=0), vmin=-0.005, vmax=0.005, cmap='seismic')
...
cnt_lv = np.arange(-0.005, 0.005, 0.0005)
...
q = ax.quiver(X[::v_zslice_int,::v_xslice_int], Z[::v_zslice_int,::v_xslice_int], 
              ...
              pivot='mid', angles='xy', scale_units='xy', scale=2.5e-6, alpha=0.6, linewidth=0.2 )
...
def update(i):
  ...
  ax.set_title(f"gravity wave ($\Theta', U', W$) t={i}[s]")
  

ani = animation.FuncAnimation(fig, update, frames=time.values, interval=1000*20)
ani.save('gravity_wave.mp4', writer="ffmpeg", fps=5)
#plt.show()

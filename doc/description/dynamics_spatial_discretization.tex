...
\section{Spatial discretization}\label{sec:dynamics_spatial_discretization}
%%
...
%%
where 
%$\bm{q}^e_{\rm dyn}=( \sqrt{G} \rho', \sqrt{G} \rho u^\xi, \sqrt{G} \rho u^\eta, \sqrt{G} \rho u^\zeta, \sqrt{G} (\rho \theta)')$ is the solution vector without tracer variables, 
$(\bm{F}_1,\bm{F}_2,\bm{F}_3)=(\bm{f}+\bm{f}_{\rm SGS},\bm{g}+\bm{g}_{\rm SGS},\bm{h}+\bm{h}_{\rm SGS})$ is the flux vector tensor, 
